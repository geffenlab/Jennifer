function [SpkTime_Laser, SpkTime_NoLaser, fList, aList] = SpikeTime(afo,SpikeData,nRep,Win);
...
Time = 0:0.5:400; %Each repetition is 400 seconds long, each trial is 500ms long
Time = Time(1:end-1);
...
for i = 1:nRep
    a = find(SpikeData(4,:) == i); %Find spikes that occur in trial j
    SpikeT = SpikeData(3,a); %Time of spikes that occur within trial j
    for j = 1:length(Time)
        b = find(SpikeT > (Time(j) - Win) & SpikeT <= (Time(j) + Win));
        if ~isempty(b)
            ResponseT{i,j} = SpikeT(b) - Time(j); %Time of spike occurance after tone onset.
        end
    end   
end 

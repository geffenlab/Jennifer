function [output] = FMsweepTC(FileName, win, binsize, smoothWin)
%Calculate tuning of FM sweep stimulus J001
%
%Inputs:
...
load('D:\Code\TuningCurve\J001F105') % Load FM sweep stimulus (although labeled F105, this does not change stimulus order so it doesn't matter F105 or F104)
load(FileName);  
...
if ~isempty(SpikeData)
    ...
    offset = abs(win(1)/binsize); %Bin offset for plotting stimulus locations
        ...
        %figure; 
        subplot(3,2,1);
        imagesc(FR_NOLASER, [0 LIM]);
        hold on 
        for i = 1:length(Stims)
            [~, StimBin(i)] = min(abs([0:binsize:1] - StimsLen(i))); %Length of stimulus (in bins)
        ...
        line([offset + 0.65/binsize offset + 0.65/binsize], [0.5 length(Stims)+0.5], 'Color','r') %Laser offset (750ms duration)
        hold off        
        set(gca,'TickDir','out', 'YDir','normal','YTick', 2:2:28,'YTickLabel',[-7.5:1:-1.5 2:1:8],'XTick',0.1/binsize:0.1/binsize:isi/binsize,'XTickLabel',-0.1:0.1:0.8)
        ...
        line([offset + 0.65/binsize offset + 0.65/binsize], [0 PUSH(end)], 'Color','r','LineStyle','--') %Laser offset (750ms duration)
        set(gca,'TickDir','out', 'XTick',0.1/binsize:0.1/binsize:isi/binsize,'XTickLabel',-0.1:0.1:0.8)
        ...
        BaseWin = 0.2; %Window to use as baseline
        BASELINE = FR_NOLASER(:,offset - BaseWin/binsize + 1:offset - 1);
        meanSPONT = mean(BASELINE(:));
        stdSPONT = std(BASELINE(:));
        for i = 1:length(Stims)
            meanOFF(i) = mean(FR_NOLASER(i,offset:StimBin(i) + offset + PostWin/binsize)) - meanSPONT;
            meanON(i) = mean(FR_LASER(i,offset:StimBin(i) + offset + PostWin/binsize))- meanSPONT;
        ...
        for i = 1:length(Stims)
            [~, StimBin2(i)] = min(abs([0:binsize2:1] - StimsLen(i))); %Length of stimulus (in bins)
        ...
        for i = 1:length(Stims)
            if ~isempty(idx_OFF{i}) 
                ...
                %normFR_OFF(i) = sum(f1_nolaser(i,idx_OFF{i}) - mean_OFF);
            end
            if ~isempty(idx_ON{i})
                meanFR_ON = mean(f1_laser(i,idx_ON{i}));
                normFR_ON(i) = (meanFR_ON - mean_OFF);
                %normFR_ON(i) = sum(f1_laser(i,idx_ON{i}) - mean_OFF);
            end
        end
    ...
    %smoothNormFR_ON = SmoothGaus(normFR_ON,2);
    %smoothNormFR_OFF = SmoothGaus(normFR_OFF,2);
    
    N_off = length(Stims);
    N_on = length(Stims);
    %tuneIDX(1) = (N_off/(N_off - 1))*(1 - mean(smoothNormFR_OFF)/max(smoothNormFR_OFF));
    %tuneIDX(2) = (N_on/(N_on - 1))*(1 - mean(smoothNormFR_ON)/max(smoothNormFR_ON));
    tuneIDX(1) = Sparseness(normFR_OFF,N_off);
    tuneIDX(2) = Sparseness(normFR_ON,N_on);
    
    % Calculate FM tuning index (sparseness)
    %[tuneIDXboot, dirIDXboot] = BootstrapFM(SpkTime_NoLaser,SpkTime_Laser,StimBin2,offset2,binsize2,PostWin,length(Stims),nRep,t);
    ...
    %output.bootTUNE = tuneIDXboot;
    %output.bootDIR = dirIDXboot;
...
% function [tuneIDX, dirIDX] = BootstrapFM(SpkTime_NoLaser,SpkTime_Laser,StimBin,offset,binsize,PostWin,nStim,nRep,t);
% 
% nBootstrap = 100;
% for u = 1:nBootstrap
%     
%     %Shuffle trials
%     for k = 1:nRep
%         SHUFFLE1 = randperm(nStim);
%         SHUFFLE2 = randperm(nStim);
%         for w = 1:nStim
%             shufSpkTime_NoLaser{w,k} = SpkTime_NoLaser{SHUFFLE1(w),k};
%             shufSpkTime_Laser{w,k} = SpkTime_Laser{SHUFFLE2(w),k};
%         end
%     end
%     
%     SpikeDataNoLaser = cell(1,nStim);
%     SpikeDataLaser = cell(1,nStim);
%     for i = 1:nStim
%         for j = 1:nRep
%             tmp1 = [shufSpkTime_NoLaser{i,j}; j*ones(1,length(shufSpkTime_NoLaser{i,j}))]; %Add a "trial #" so final format will be similar to SpikeData arrays
%             tmp2 = [shufSpkTime_Laser{i,j}; j*ones(1,length(shufSpkTime_Laser{i,j}))]; %Add a "trial #" so final format will be similar to SpikeData arrays
%             SpikeDataNoLaser{i} = [SpikeDataNoLaser{i} tmp1];           
%             SpikeDataLaser{i} = [SpikeDataLaser{i} tmp2];
%         end
%     end
%     
%      for i = 1:nStim
%             f_nolaser = histc(SpikeDataNoLaser{i}(1,:), t)/(nRep*binsize);
%             f_laser = histc(SpikeDataLaser{i}(1,:), t)/(nRep*binsize);
%             f1_nolaser(i,:) = f_nolaser(1:end-1);
%             f1_laser(i,:) = f_laser(1:end-1);
%      end
% 
%     %STEP 2: Take mean and std for 200ms before sweep onset
% 
%     f1_spont = f1_nolaser(:,1:offset-1);
%     std_OFF = std(f1_spont(:));
%     mean_OFF = mean(f1_spont(:));
% 
%     %STEP 3: Find bins for which spike count exceeds 95% confidence limit
...
%     ConfLim = 1.96.*std_OFF;
%     for i = 1:nStim
%         indbins = offset:StimBin(i) + PostWin/binsize + offset; %Indices of sweep + 50ms post-sweep
%         idx_OFF{i} = find(f1_nolaser(i,indbins) > mean_OFF + ConfLim | f1_nolaser(i,indbins) < mean_OFF - ConfLim);
%         idx_OFF{i} = idx_OFF{i} + offset - 1;
%         idx_ON{i} = find(f1_laser(i,indbins) > mean_OFF + ConfLim | f1_laser(i,indbins) < mean_OFF - ConfLim);
%         idx_ON{i} = idx_ON{i} + offset - 1;
%     end
% 
%     normFR_OFF = zeros(1,nStim);
%     normFR_ON = zeros(1,nStim);
%     for i = 1:nStim
%         if ~isempty(idx_OFF{i}) 
%             meanFR_OFF = mean(f1_nolaser(i,idx_OFF{i}));
%             normFR_OFF(i) = (meanFR_OFF - mean_OFF);
...
%     N_on = nStim;
% 
%     %tuneIDX(u,1) = (N_off/(N_off - 1))*(1 - mean(smoothNormFR_OFF)/max(smoothNormFR_OFF));
%     %tuneIDX(u,2) = (N_on/(N_on - 1))*(1 - mean(smoothNormFR_ON)/max(smoothNormFR_ON));
%     
%     tuneIDX(u,1) = Sparseness(normFR_OFF,N_off);
%     tuneIDX(u,2) = Sparseness(normFR_ON,N_on);
%     
%     downOFF = mean(smoothNormFR_OFF(1:14));
%     upOFF = mean(smoothNormFR_OFF(15:end));
%     downON = mean(smoothNormFR_ON(1:14));
%     upON = mean(smoothNormFR_ON(15:end));
%     
%     dirIDX(u,1) = (upOFF - downOFF)/(upOFF + downOFF);
%     dirIDX(u,2) = (upON - downON)/(upON + upOFF);
%         
% end
%end

function smoothedFR = smoothFRx2(SpikeData,nRep,binsize,Win,gwin)
%Smooths spike data by convolving firing rates with a gaussian filter


%Input:
    %SpikeData = matrix [time stamp; time (relative to exp start); time
    %(relative to trial onset); trial #]
...
f = histc(SpikeData(3, :), t)/(nRep*binsize); %dividing by nRep averages the number of spikes over the trials instead of just summing them and dividing by binsize makes it spikes per second instead of spikes per 5ms.

function [FR_NOLASER, FR_LASER, CellQ] = smoothTC(MNum,file,nfreq,amps)
...
%   file: string
%       Contains file name string of format TC3-105-MXXXX...._laser.mat
%   nfreq: integer
%       Number of frequencies to use in smoothed tuning curve
%   amps: integer/vector
%       Designates the indices of which amplitudes to use in smoothed
%       tuning curve (some combination of 1 though 8)
%
% Returns
% -------
%   FR_NOLASER: vector
%       Contains smoothed firing tuning curve for no laser condition
%   FR_LASER: vector
%       Contains smoothed firing tuning curve for laser condition 
%   CellQ: integer
%       Cell quality
    ...
    [LOCSon,LOCSoff] = TC_Select_noGauss(MNum,file,nfreq,0);
    ...
    match = strfind(file, '0');
    q = match(end);
    load(['D:\Spikes\M' num2str(MNum) '\SpikeMat\R407F'  file(5:q) '.mat']); 
    CellQ = CellInfo(6);
